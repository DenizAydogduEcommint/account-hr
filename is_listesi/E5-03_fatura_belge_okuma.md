...
- **Frontend entegrasyonu (2026-06-29):** Fatura yükleme modalında PDF seçilince `POST /invoices/parse` otomatik çağrılır → "Okunan Bilgiler" paneli (no/tarih/tutar/KDV/sağlayıcı) + **KDV oranı otomatik doldurulur** (kullanıcı dokunmadıysa). Hata-toleranslı (parse fail → sessiz not, yükleme bloklanmaz), stale-guard (dosya değişince eski yanıt yok sayılır). **Gerçek tarayıcı doğrulaması (Playwright):** OpenAI faturası yüklendi → panel doldu (E15F9A97-0013 / 07.10.2025 / 36,00 USD / %20·6,00 / OpenAI, LLC), KDV otomatik %20 seçildi, konsol 0 hata.
- **Kalan:** JPG→OCR (Tesseract); parse sonucunu eşleştirme akışına bağlama (E5-04).
